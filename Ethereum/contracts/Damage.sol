...
contract Damage {
    ...
    function getAllData(uint _index)public view returns(string memory, string memory, string memory, string memory, int){
        //Validation Check for Recieved Index
        if (_index <= damageCount && _index != 0) {
            //Everything sent except DateArray
            return (reported[_index].damType,reported[_index].URL, reported[_index].lat, reported[_index].long, reported[_index].priority);
        }
        //Default blank values will be sent if index isnt found
        return ("", "", "", "", 0);
    }
    ...
    function saveDeleteHistory(uint _index) private {
        ...
        deleted[deleteCount] = RoadDamage(reported[_index].damType, reported[_index].URL, reported[_index].lat, reported[_index].long, new string[](0), 0, 0);
    ...
    function undoDelete(uint _index) public {
        ...
        if (_index <= deleteCount && _index != 0) {
            damageCount ++;
            //New Damage Instance Created
            reported[damageCount] = RoadDamage(deleted[_index].damType, deleted[_index].URL, deleted[_index].lat, deleted[_index].long, new string[](0), 0, 0);
            ...
            deleted[_index].damageReportCount = reported[deleteCount].damageReportCount;
